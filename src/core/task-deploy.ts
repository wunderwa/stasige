...
import { CoreConfig, DeployConfig } from './utils/types.js'
import path from 'path'
const CMD = 'deploy'
const onError = (error: string) => printHelp(CMD, { error })

export const taskDeploy = async (config: CoreConfig) => {
  ...
  const deployConfig = readConfig<DeployConfig>(
    deployConfigPath,
  ) as DeployConfig

  if (!config) onError(`Does not have deploy configuration for '${mode}' mode`)
  ...
  if (!sshConf) onError(`Does not have ssh configuration for '${mode}' mode`)
  if (!sshPath) onError(`Does not have ssh path for '${mode}' mode`)
